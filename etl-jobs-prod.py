...
PG_DBNAME = os.getenv("PG_DBNAME", "prod_etl_db")
...
for doc in db[JOBS_COLLECTION].find():
    ...
    # Ensure datetime-safe conversions
    def parse_dt(value):
        if not value:
            return None
    ...
    scheduleStart = parse_dt(doc.get("scheduleStart"))
    scheduleEnd = parse_dt(doc.get("scheduleEnd"))
    pickupTime = parse_dt(doc.get("pickupTime"))
    pickupFrequency = doc.get("pickupFrequency")
    createdAt = parse_dt(doc.get("createdAt"))
    version = doc.get("__v")
